...
const Task = ({ taskObj, onComplete }) => {
  const dateLine = formatDistanceToNow(new Date(taskObj.deadline), {
    addSuffix: true,
    locale: tr,
  });
  ...
  const urgencyClass = result <= 3 ? 'bg-urgent' : 'bg-normal';
  ...
  return (
    <div className="task p-6 bg-white rounded-md leading-normal mt-4 shadow-[0_4px_5px_0_rgb(0 0 0 / 10%)]  ">
      ...
      <div className="deadline text-xs pt-1 ">
        son teslim:{' '}
        <span className={`py-1 px-2 rounded-sm inline-block ${urgencyClass} `}>
          {dateLine}
        </span>
      </div>
      ...
      <div>
        {taskObj.people.map((p) => (
          <span
            className="pill inline-block pt-1  py-1.5 px-3 text-sm border-2 border-solid border-[#ccc] mr-1 mb-1.5 rounded-[30px] "
            key={p}
          >
            {p}
          </span>
